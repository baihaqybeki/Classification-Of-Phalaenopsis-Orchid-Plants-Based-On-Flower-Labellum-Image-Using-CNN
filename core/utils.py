...
def draw_roi(frame):
    ...
    width = frame.shape[1]
    roi_position = int(width/2)
    print("roi",roi_position)
    #point (x,y)
    cv2.line(img=frame, pt1=(roi_position,0), pt2=(roi_position, height), color=(255, 0, 0), thickness=5)
    return roi_position
...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    ...
    for i in range(num_boxes):
        ...
        left, right, top, bottom = int(coor[0]), int(coor[2]),int(coor[1]), int(coor[3])
        a = abs((right+left)/2-320)
        if a < 10:
            print("centroid",a)
